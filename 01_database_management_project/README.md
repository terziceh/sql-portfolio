...
**Database Platform:** Oracle SQL (Oracle SQL Developer / Oracle APEX syntax)
...
This project emphasizes database design, normalization, and SQL querying rather than application-layer development.
...
## Database Design
The schema follows Third Normal Form (3NF) to reduce redundancy and maintain data integrity.
...
An Entity Relationship Diagram (ERD) is stored in the `schema/` folder.
...
- Identify documents linked to internship applications
- Display reminders due within the next 7 days
- Show pending applications and associated documents
...
- Indexed application status for faster filtering
- Views for common access patterns (student dashboards, application summaries)
...
- Schema is optimized for transactional use; analytics would benefit from a data warehouse
